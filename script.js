/* ================================================================
PICAZO — script.js  (Updated)
✅ Public defaults: 3 rounds, 90s draw time, 7 bots (up to 12 players)
✅ Timer fixed — counts down correctly
✅ Min 3 rounds enforced
✅ Hint intensity: SLOW (reveals at 40% and 15% time remaining)
✅ Bots always eventually guess correctly so game completes
✅ Glassmorphism toolbar with Shape Selector (rect/circle/line/triangle)
...
// Vector avatar definitions — drawn on canvas, no external API
const AVATAR_DEFS = [
...
/* ================================================================
AVATAR RENDERER (Vector, canvas-drawn)
================================================================ */
...
[-1, 1].forEach(s => {
c.beginPath();
c.ellipse(cx + s * headR * 0.92, headY + headR * 0.05, headR * 0.2, headR * 0.25, 0, 0, Math.PI * 2);
c.fill();
});
...
c.beginPath(); c.ellipse(cx, headY - headR * 0.7, headR * 0.95, headR * 0.48, 0, Math.PI, Math.PI * 2); c.fill();
...
/* ================================================================
GAME STATE
Public game defaults: 3 rounds, 90s draw time, 7 bots, slow hints
================================================================ */
let S = {
avatarIdx: 0,
playerName: ‘’,
totalRounds: 3,     // default 3 rounds minimum
drawTime: 90,       // default 90s
botCount: 7,        // 7 bots + 1 player = 8 total (up to 12 max)
...
isDrawing: false,
tool: ‘pencil’,
shape: ‘rect’,      // current shape for shape tool
color: ‘#000000’,
brushSize: 3,
strokes: [],
shapeStart: null,
...
// Circumference for r=23 timer ring
...
// Keep select elements synced but hidden — public game uses fixed defaults
selRounds.addEventListener(‘change’, e => { S.totalRounds = Math.max(3, +e.target.value); });
...
S.playerName = name;
// Fixed public defaults — ignore hidden selects
S.totalRounds = 3;
S.drawTime    = 90;
S.botCount    = 7;
transitionToGame();
...
if (isMobile) {
if (!tRow) { tRow = document.createElement(‘div’); tRow.className = ‘top-row-mobile’; game.insertBefore(tRow, body); }
if (!bRow) { bRow = document.createElement(‘div’); bRow.className = ‘bottom-row-mobile’; game.insertBefore(bRow, body); }
...
if (tRow) { game.insertBefore(header, body); body.insertBefore(canvasCol, body.firstChild); tRow.remove(); }
if (bRow) { body.insertBefore(lb, body.firstChild); body.appendChild(chat); bRow.remove(); }
...
PLAYERS & LEADERBOARD
================================================================ */
const BOT_NAMES = [‘SketchBot’,‘ArtGeek’,‘DrawMaster’,‘DoodleKing’,‘PicassoJr’,‘BrushWizard’,‘InkMage’,‘PixelPro’,‘ColorWhiz’,‘LineArtist’];
...
// If not drawer, auto-pick after 4s (bot picks for the game flow)
if (!S.isDrawer) {
setTimeout(() => {
...
`<div class="wb-line" style="width:${Math.max(18, Math.floor(96 / S.currentWord.length))}px"></div>`);
wordDisplay.appendChild(grp);
}
wordMeta.textContent = S.isDrawer
? `You are drawing — ${S.currentWord.length} letters`
: `${S.currentWord.length} letters`;
}

function revealHintLetter() {
const unrevealed = S.currentWord.split(’’)
.map((c, i) => i)
...
// ── TIMER FIX: proper interval, correct dashoffset, slow hints at 40% and 15% ──
function startRoundTimer() {
S.timeLeft = S.drawTime;
clearInterval(S.timerInterval);
updateTimerUI();

// Slow hint intensity: reveal at 40% and 15% of time remaining
const hint1 = Math.floor(S.drawTime * 0.40);
const hint2 = Math.floor(S.drawTime * 0.15);

S.timerInterval = setInterval(() => {
S.timeLeft–;
if (S.timeLeft === hint1 || S.timeLeft === hint2) revealHintLetter();
updateTimerUI();
if (S.timeLeft <= 0) {
clearInterval(S.timerInterval);
endRound(false);
...
const progress = S.drawTime > 0 ? (S.timeLeft / S.drawTime) : 0;
tFg.style.strokeDashoffset = CIRC * (1 - progress);
...
`<div class="re-score-row"> <span class="re-score-name">${i===0?'🥇':i===1?'🥈':i===2?'🥉':''} ${escHtml(p.name)}</span> <span class="re-score-pts">${p.score} pts</span> </div>`).join(’’);
...
/* ================================================================
BOT GUESSING — bots always guess correctly to complete game
================================================================ */
function scheduleBotGuesses() {
const bots = S.players.filter(p => !p.isSelf && p.id !== S.players[S.drawerIdx]?.id);
const fakeGuesses = [‘hmm…’,‘a house?’,‘cat!’,‘maybe a car?’,‘tree?’,‘fish!’,‘airplane?’,‘dog?’,‘flower?’,‘mountain?’,‘boat?’,‘sun?’];

bots.forEach((bot, idx) => {
// Each bot makes 1-2 wrong guesses, then always guesses correctly
const wrongCount = 1 + Math.floor(Math.random() * 2);

```
// Wrong guesses first
for (let w = 0; w < wrongCount; w++) {
  const wrongDelay = 4000 + idx * 2500 + w * 3500 + Math.random() * 2000;
  setTimeout(() => {
...
// Correct guess — staggered so they don't all guess at once
// Bots spread out between 30% and 80% of draw time
const correctFraction = 0.30 + (idx / Math.max(bots.length - 1, 1)) * 0.50;
const correctDelay = (S.drawTime * (1 - correctFraction)) * 1000 + Math.random() * 3000;

setTimeout(() => {
  if (!S.currentWord || bot.guessed || S.timeLeft <= 0) return;
  botGuessCorrect(bot);
}, correctDelay);
```
...
if (bot.guessed || !S.currentWord) return;
...
if (S.tool === ‘rect’ || S.tool === ‘circle’ || S.tool === ‘line’ || S.tool === ‘triangle’) {
S.shapeStart = pos;
...
applyBrushStyle();
drawShape(S.tool, S.shapeStart, pos);
return;
}
...
/* ================================================================
TOOLBAR — Glassmorphism popups with all tools functional
================================================================ */
...
const dot = $(‘size-dot-preview’);
const px = Math.min(S.brushSize * 1.5, 36);
...
e.stopPropagation();
pShape.classList.toggle(‘hidden’);
pColor.classList.add(‘hidden’);
pSize.classList.add(‘hidden’);
});

// Shape buttons
[‘rect’,‘circle’,‘line’,‘triangle’].forEach(shp => {
$(‘shape-’ + shp).addEventListener(‘click’, () => {
S.tool  = shp;
S.shape = shp;
document.querySelectorAll(’.shape-btn’).forEach(b => b.classList.toggle(‘active’, b.id === ‘shape-’ + shp));
document.querySelectorAll(’.tool-btn[data-tool]’).forEach(b => b.classList.remove(‘active’));
$(‘btn-shape-popup’).classList.add(‘active’);
gameCanvas.className = ‘’;
pShape.classList.add(‘hidden’);
});
});

// Close popups on outside click / canvas touch
document.addEventListener(‘click’, e => {
if (!pColor.contains(e.target) && e.target.id !== ‘btn-color-popup’ && !$(‘btn-color-popup’).contains(e.target)) pColor.classList.add(‘hidden’);
if (!pSize.contains(e.target)  && e.target.id !== ‘btn-size-popup’  && !$(‘btn-size-popup’).contains(e.target))  pSize.classList.add(‘hidden’);
if (!pShape.contains(e.target) && e.target.id !== ‘btn-shape-popup’ && !$(‘btn-shape-popup’).contains(e.target)) pShape.classList.add(‘hidden’);
});
...
$(‘btn-shape-popup’).classList.remove(‘active’);
